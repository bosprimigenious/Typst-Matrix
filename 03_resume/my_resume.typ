// SixtySecondsCV 双栏，依赖 00_core_engine/theme.typ
...
#let sidecolor = colors.bg-canvas
#let sectioncolor = colors.primary
#let subsectioncolor = colors.text-muted
#let iconcolor = colors.primary
#let itemtextcolor = colors.text-main
...
#let skill-points(icon, name, level, max: 5) = {
  grid(columns: (1.5em, 1fr, auto), column-gutter: 0.4em, align: left)[
    ...
    #grid(columns: max, column-gutter: 0.15em, ..range(max).map(i => circle(radius: 0.2em, fill: if i < level { iconcolor } else { colors.border })))
  ]
  v(0.4em)
}
...
#let skill-bar(name, level-percent) = {
  block(width: 100%, below: 0.4em)[
    #text(size: 8.5pt)[#name]
    #v(0.2em)
    #block(width: 100%, height: 0.5em, fill: colors.border, radius: 0.25em, clip: true)[
