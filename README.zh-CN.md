...
- **Data-Driven / 数据驱动：** 配置即内容，用 TOML/YAML 驱动渲染，样式与数据解耦。
- **Zero-Setup CI/CD / 零配置流水线：** 云端编译并自动发布到 Releases。
- **Bilingual Support / 原生双语：** 引擎层内置语言路由（`lang: "zh" | "en"`），支持中英字体回退与断行。
- **Modular Design System / 模块化设计：** 统一的全局色彩面板（Slate & Navy）与组件库。
